...
func vyosRuleSummary(chain *vyos.Node) []string {
	...
	for _, key := range chain.ContainerKeys() {
		...
		if err != nil || number < vyosHostRuleStart {
			continue
		}
...
func TestVyosGoldenConfigs(t *testing.T) {
	...
	if got := generator.Routers(); !reflect.DeepEqual(got, []string{"r-us-tst-5-8", "r-us-tst-5-9"}) {
...
func TestVyosRouterConventions(t *testing.T) {
	...
	cases := []struct {
		...
		{[]string{"interfaces", "ethernet", "eth1", "address"}, []string{"203.0.113.81/27", "2001:db8:99::58/48"}},
	...
	otherCases := []struct {
		...
		{[]string{"interfaces", "ethernet", "eth1", "address"}, []string{"203.0.113.89/27", "2001:db8:99::59/48"}},
...
func TestVyosRulesForAnLbInterface(t *testing.T) {
	...
	for _, chain := range []string{"WAN_LOCAL", "WANv6_LOCAL"} {
		...
		if got := local.ContainerKeys(); !reflect.DeepEqual(got, []vyos.Key{{Name: "rule", Tag: "10"}, {Name: "rule", Tag: "20"}, {Name: "rule", Tag: "30"}}) {
...
func TestVyosInterfaceForwardsBecomeRouterDnat(t *testing.T) {
	...
	if got := nat.ContainerKeys(); !reflect.DeepEqual(got, []vyos.Key{{Name: "rule", Tag: "100"}, {Name: "rule", Tag: "5000"}, {Name: "rule", Tag: "5001"}}) {
		t.Fatalf("nat rules = %v", got)
	}
...
func TestVyosAddressDerivation(t *testing.T) {
	...
	if err != nil || wan.String() != "2001:db8:99::58/48" {
