...
// forward ingester drop descriptors into the existing HTTP response, and use
// the existing processing queue before dropping a short producer burst. The
...
func TestGrafanaImageBuildsPatchedLokiTailClose(t *testing.T) {
	...
	for _, required := range []string{
		...
		"-run '^TestTailer(ResponseHandoffAbsorbsShortBurst|GenuineBackpressureRemainsBounded|CanceledContextUnblocksResponseHandoff)$'",
		"-run '^Test(TailerCloseIsIdempotent|TailClientReceiveErrorIncludesBackendAddress|TailerForwardsIngesterDroppedStreams)$'",
		"github.com/grafana/loki/v3/pkg/util/build.Version=${loki_version}-urnetwork.5",
		"github.com/grafana/loki/v3/pkg/util/build.Revision=82cdcdc0+tail-close-once+tail-backend-addr+forward-ingester-drops+tail-burst-handoff+quiet-table-lookups",
	...
	for _, required := range []string{
		...
		"case <-t.closeChan:",
		"func TestTailerResponseHandoffAbsorbsShortBurst",
		"func TestTailerGenuineBackpressureRemainsBounded",
		"func TestTailerCanceledContextUnblocksResponseHandoff",
