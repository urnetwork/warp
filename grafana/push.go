...
var (
	mimirSeriesLimitBodyRe    = regexp.MustCompile(`^(?:(?:send data to ingesters: )?failed pushing to ingester [^[:space:]]+: )?(?:user=[^[:space:]:]+: )?per-user series limit of [1-9][0-9]* exceeded \(err-mimir-max-series-per-user\)\. To adjust the related per-tenant limit, configure -ingester\.max-global-series-per-user, or contact your service administrator\.$`)
